...
def _comp_point_coordinate(self):
    ...
    Z0 = 1j * self.H1 / (2 * sin(self.W0 / 2))
    Z1 = -self.W3 / 2 + 1j * (Z0.imag + (self.W3 / 2) / tan(self.W0 / 2))
    Z2 = (
        Z1.real
        - sin(self.W0 / 2) * (self.W2 - self.H1)
        + 1j * (Z1.imag + cos(self.W0 / 2) * (self.W2 - self.H1))
    )

    # Draw the left hole with center P2
    Z3h = -self.H1 / 2 - 1j * self.H1 / 2
    Z4h = -1j * self.H1
    Z5h = self.W2 - self.H1 - 1j * self.H1
    Z6h = self.W2 - self.H1 / 2 - 1j * (self.H1 / 2)
    w = self.W2 - self.H0
    ZM1h = (w - self.W1) / 2 + self.W1
    ZM2h = (w - self.W1) / 2
    ZM4h = (w - self.W1) / 2 + self.W1 - 1j * self.H1
    ZM3h = (w - self.W1) / 2 - 1j * self.H1
