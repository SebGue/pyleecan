...
from ....Functions.labels import HOLEM_LAB, HOLE_LAB, MAG_LAB, ROTOR_LAB
...
def build_geometry(self, alpha=0, delta=0, is_simplified=False):
    ...
    if is_radius:
        curve_list.append(
            Arc1(Z1R, Z1L, self.R0, prop_dict={BND: "_".join([HOLE, TOP_LAB, "0"])})
        )
    ...
    curve_list.append(
        Segment(
            Z11,
            Z1R if is_radius else Z1,
            prop_dict={BND: "_".join([HOLE, TOP_LAB, "0"])},
        )
    )
    ...
    if is_simplified:
        ...
        curve_list.append(
            Segment(Z4, Z3, prop_dict={BND: "_".join([MAG, LEFT_LAB, "0"])})
        )
        curve_list.append(
            Segment(Z3, Z11, prop_dict={BND: "_".join([MAG, LEFT_LAB, "0"])})
        )
        curve_list.append(
            Segment(Z11, Z10, prop_dict={BND: "_".join([MAG, TOP_LAB, "0"])})
        )
        curve_list.append(
            Segment(Z10, Z7, prop_dict={BND: "_".join([MAG, RIGHT_LAB, "0"])})
        )
        curve_list.append(
            Segment(Z7, Z6, prop_dict={BND: "_".join([MAG, RIGHT_LAB, "0"])})
        )
        curve_list.append(
            Segment(Z6, Z4, prop_dict={BND: "_".join([MAG, BOT_LAB, "0"])})
        )
    ...
    if is_radius:
        curve_list.append(
            Arc1(Z9R, Z9L, self.R0, prop_dict={BND: "_".join([HOLE, TOP_LAB, "0"])})
        )
    curve_list.append(
        Segment(
            Z9L if is_radius else Z9,
            Z10,
            prop_dict={BND: "_".join([HOLE, TOP_LAB, "0"])},
        )
    )
