...
def save_mesh(permeabiltiy_materials, Num_Unknowns, list_elem, x, y,BC_list):
    ...
    np.savetxt(f_handle, np.column_stack((list_elem, permeabiltiy_materials)),
               fmt="%u", header=str(permeabiltiy_materials.size)+' '+str(Num_Unknowns.size))
    ...
    np.savetxt(f_handle, np.column_stack(
        (list_coord, Num_Unknowns,BC_list)), fmt="%f %f %d %d")
    f_handle.close()
    print("mesh is saved")
