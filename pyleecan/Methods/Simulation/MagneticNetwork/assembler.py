...
def assembly(self, reluc_list, Num_Unknowns, list_elem, permeability_cell, BC_list):
    ...
    R_N = self.reluc_list[:, 0] / self.permeability_cell
    R_E = self.reluc_list[:, 1] / self.permeability_cell
    R_S = self.reluc_list[:, 2] / self.permeability_cell
    R_W = self.reluc_list[:, 3] / self.permeability_cell

    # Select indices of each vertices of this elements (square)
    i1 = self.Num_Unknowns[self.list_elem[:, 0]]
    mask_i1 = i1 != -1
    i2 = self.Num_Unknowns[self.list_elem[:, 1]]
    mask_i2 = i2 != -1
    i3 = self.Num_Unknowns[self.list_elem[:, 2]]
    mask_i3 = i3 != -1
    i4 = self.Num_Unknowns[self.list_elem[:, 3]]
    mask_i4 = i4 != -1

    t1 = time.perf_counter()

    # Add tirangular part
    if np.all(self.BC_list != 3):
        ...
        mask_BC1 = self.BC_list[self.list_elem[:, 0]] == 3
        mask_BC2 = self.BC_list[self.list_elem[:, 1]] == 3
        mask_BC3 = self.BC_list[self.list_elem[:, 2]] == 3
        mask_BC4 = self.BC_list[self.list_elem[:, 3]] == 3
...
def assembly_one_area(
    ...
    self.permeability_cell = self.permeability_cell[mask_area]
    self.reluc_list = self.reluc_list[mask_area]
    ...
    i1 = self.Num_Unknowns[self.list_elem[mask_area, 0]]
    mask_i1 = i1 != -1

    i2 = self.Num_Unknowns[self.list_elem[mask_area, 1]]
    mask_i2 = i2 != -1

    i3 = self.Num_Unknowns[self.list_elem[mask_area, 2]]
    mask_i3 = i3 != -1

    i4 = self.Num_Unknowns[self.list_elem[mask_area, 3]]
    mask_i4 = i4 != -1

    if np.all(self.BC_list[self.list_elem[mask_area, :]] != 3):
        ...
        mask_BC1 = self.BC_list[self.list_elem[mask_area, 0]] == 3
        mask_BC2 = self.BC_list[self.list_elem[mask_area, 1]] == 3
        mask_BC3 = self.BC_list[self.list_elem[mask_area, 2]] == 3
        mask_BC4 = self.BC_list[self.list_elem[mask_area, 3]] == 3
...
def right_member_assembly(
    ...
    # FMMPM = 0
    i1 = self.Num_Unknowns[self.list_elem[mask_magnet, 0]]
    ...
    mask_winding = self.cells_materials == 1
    S = h_x[mask_winding] * h_y[mask_winding] / 4
    i1 = self.Num_Unknowns[self.list_elem[mask_winding, 0]]
    i2 = self.Num_Unknowns[self.list_elem[mask_winding, 1]]
    i3 = self.Num_Unknowns[self.list_elem[mask_winding, 2]]
    i4 = self.Num_Unknowns[self.list_elem[mask_winding, 3]]
    ...
    mask_winding = self.cells_materials == 2
    S = h_x[mask_winding] * h_y[mask_winding] / 4
    i1 = self.Num_Unknowns[self.list_elem[mask_winding, 0]]
    i2 = self.Num_Unknowns[self.list_elem[mask_winding, 1]]
    i3 = self.Num_Unknowns[self.list_elem[mask_winding, 2]]
    ...
    mask_winding = self.cells_materials == 3
    S = h_x[mask_winding] * h_y[mask_winding] / 4
    i1 = self.Num_Unknowns[self.list_elem[mask_winding, 0]]
    i2 = self.Num_Unknowns[self.list_elem[mask_winding, 1]]
    i3 = self.Num_Unknowns[self.list_elem[mask_winding, 2]]
