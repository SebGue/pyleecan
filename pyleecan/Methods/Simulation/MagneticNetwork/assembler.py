...
def assembly(reluc_list, Num_Unknowns, list_elem, permeability_cell,BC_list):
    ...
    if np.all(BC_list!=3):
        ...
        I = i1[mask]
        J = i2[mask]
        data = R_S[mask]*plus_minus[(np.logical_xor(mask_BC1,mask_BC2)).astype(np.int32)][mask]
        graph = coo_matrix((data, (I,J)), shape=(nn, nn))
    
        mask = mask_i2*mask_i3
        I = i2[mask]
        J = i3[mask]
        data = R_E[mask]*plus_minus[(np.logical_xor(mask_BC2,mask_BC3)).astype(np.int32)][mask]
        graph += coo_matrix((data, (I,J)), shape=(nn, nn))
    
        mask = mask_i3*mask_i4
        I = i3[mask]
        J = i4[mask]
        data = R_N[mask]*plus_minus[(np.logical_xor(mask_BC3,mask_BC4)).astype(np.int32)][mask]
        graph += coo_matrix((data, (I, J)), shape=(nn, nn))
    
        mask = mask_i4*mask_i1
        I = i4[mask]
        J = i1[mask]
        data = R_W[mask]*plus_minus[(np.logical_xor(mask_BC4,mask_BC1)).astype(np.int32)][mask]
        graph += coo_matrix((data, (I, J)), shape=(nn, nn))
...
def assembly_one_area(area_selected, reluc_list,list_geometry, Num_Unknowns, list_elem, permeability_cell,BC_list):
    """
...
def right_member_assembly(cells_materials, Num_Unknowns, list_elem,list_coord,Br,mu0,mode):
    """
