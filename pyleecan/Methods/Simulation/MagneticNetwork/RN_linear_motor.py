...
def linear_model(size_x, size_y, x, y, x_dual, y_dual, pos, BC):
    ...
    permeability_cell = init_permeabilty_cell(
        size_x, size_y, permeabilty_materials, list_geometry
    )
...
def mu_non_linear(
    Phi, permeability_cell, list_geometry, Num_Unknowns, list_elem, BC_list
):
    # Compute staured mu
    B_sat = 1.8
    materials = 2
...
def non_linear_model(size_x, size_y, x, y, x_dual, y_dual, pos, sol0, BC):
    ...
    while criteria > eps and i < 500:
        ...
        permeability_cell = mu_non_linear(
            F, permeability_cell, list_geometry, Num_Unknowns, list_elem, BC_list
        )
        # Update matrix
        M3 = assembly_one_area(
            2,
            list_materials,
            permeabilty_materials,
            size_x,
    ...
    print(
        ...
        np.round(t2 - t1, 5),
        "secondes, res:",
        np.linalg.norm((M2 + M3) @ F - E, ord=2),
    )
