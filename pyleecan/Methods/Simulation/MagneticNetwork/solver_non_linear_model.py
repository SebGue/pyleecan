...
from pre_processing import save_mesh,init_reluc,init_point,init_cell,init_permeabilty_cell,init_permeabilty_cell,init_mesh_BC,numeroting_unknows
...
def mu_non_linear(Phi, permeability_cell, list_geometry,Num_Unknowns,list_elem,BC_list,list_coord,la,mu0):
    """
...
def non_linear_model(size_x, size_y, x, y, pos,geometry, sol0,BC,la,mu0,Br):
    ...
    permeability_cell=init_permeabilty_cell(size_x,size_y,permeability_materials,list_geometry)
    ...
    # Assembly RHS
    E = right_member_assembly(list_geometry, Num_Unknowns, list_elem,list_coord,Br,mu0)
    ...
    for i in range(0,permeability_materials.size):
        Matrix=assembly_one_area(i+1, reluc_list,list_geometry, Num_Unknowns, list_elem, permeability_cell,BC_list)
    ...
    while criteria2 > eps and i < 500:
        print("loop n°", i+1)
        # Update permeability
        permeability_cell = mu_non_linear(F, permeability_cell, list_geometry,Num_Unknowns,list_elem,BC_list,list_coord,la,mu0)
        # Update matrix
        M_non_lin= csr_matrix((nn,nn), dtype=np.float64)
        
        for j in non_linear_area:
            Matrix=assembly_one_area(j, reluc_list,list_geometry, Num_Unknowns, list_elem, permeability_cell,BC_list)
            M_non_lin += Matrix
