# -*- coding: utf-8 -*-
# File generated according to Generator/ClassesRef/Slot/HoleM64.csv
# WARNING! All changes made in this file will be lost!
"""Method code available at https://github.com/Eomys/pyleecan/tree/master/pyleecan/Methods/Slot/HoleM64
"""
...
try:
    from ..Methods.Slot.HoleM64._comp_point_coordinate import _comp_point_coordinate
except ImportError as error:
    _comp_point_coordinate = error

try:
    from ..Methods.Slot.HoleM64.build_geometry import build_geometry
except ImportError as error:
    build_geometry = error

try:
    from ..Methods.Slot.HoleM64.plot_schematics import plot_schematics
except ImportError as error:
    plot_schematics = error

try:
    from ..Methods.Slot.HoleM64.remove_magnet import remove_magnet
except ImportError as error:
    remove_magnet = error

try:
    from ..Methods.Slot.HoleM64.comp_surface_magnet_id import comp_surface_magnet_id
except ImportError as error:
    comp_surface_magnet_id = error

try:
    from ..Methods.Slot.HoleM64.comp_radius import comp_radius
except ImportError as error:
    comp_radius = error

try:
    from ..Methods.Slot.HoleM64.has_magnet import has_magnet
except ImportError as error:
    has_magnet = error

try:
    from ..Methods.Slot.HoleM64.check import check
except ImportError as error:
    check = error

try:
    from ..Methods.Slot.HoleM64.comp_magnetization_dict import comp_magnetization_dict
except ImportError as error:
...
class HoleM64(HoleMag):
    """I shape slot for buried magnet"""

    VERSION = 1
    IS_SYMMETRICAL = 1

    # Check ImportError to remove unnecessary dependencies in unused method
    # cf Methods.Slot.HoleM64._comp_point_coordinate
    if isinstance(_comp_point_coordinate, ImportError):
        _comp_point_coordinate = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM64 method _comp_point_coordinate: "
    ...
    # cf Methods.Slot.HoleM64.build_geometry
    if isinstance(build_geometry, ImportError):
        build_geometry = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM64 method build_geometry: " + str(build_geometry)
                )
            )
        )
    else:
        build_geometry = build_geometry
    # cf Methods.Slot.HoleM64.plot_schematics
    if isinstance(plot_schematics, ImportError):
        plot_schematics = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM64 method plot_schematics: " + str(plot_schematics)
                )
            )
        )
    else:
        plot_schematics = plot_schematics
    # cf Methods.Slot.HoleM64.remove_magnet
    if isinstance(remove_magnet, ImportError):
        remove_magnet = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM64 method remove_magnet: " + str(remove_magnet)
                )
            )
        )
    else:
        remove_magnet = remove_magnet
    # cf Methods.Slot.HoleM64.comp_surface_magnet_id
    if isinstance(comp_surface_magnet_id, ImportError):
        comp_surface_magnet_id = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM64 method comp_surface_magnet_id: "
    ...
    # cf Methods.Slot.HoleM64.comp_radius
    if isinstance(comp_radius, ImportError):
        comp_radius = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM64 method comp_radius: " + str(comp_radius))
            )
        )
    else:
        comp_radius = comp_radius
    # cf Methods.Slot.HoleM64.has_magnet
    if isinstance(has_magnet, ImportError):
        has_magnet = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM64 method has_magnet: " + str(has_magnet))
            )
        )
    else:
        has_magnet = has_magnet
    # cf Methods.Slot.HoleM64.check
    if isinstance(check, ImportError):
        check = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM64 method check: " + str(check))
            )
        )
    else:
        check = check
    # cf Methods.Slot.HoleM64.comp_magnetization_dict
    if isinstance(comp_magnetization_dict, ImportError):
        comp_magnetization_dict = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM64 method comp_magnetization_dict: "
    ...
    def __str__(self):
        ...
        HoleM64_str += super(HoleM64, self).__str__()
        HoleM64_str += "W1 = " + str(self.W1) + linesep
        HoleM64_str += "W2 = " + str(self.W2) + linesep
        HoleM64_str += "W3 = " + str(self.W3) + linesep
        HoleM64_str += "H0 = " + str(self.H0) + linesep
        HoleM64_str += "H2 = " + str(self.H2) + linesep
        HoleM64_str += "R0 = " + str(self.R0) + linesep
        if self.magnet_0 is not None:
            tmp = self.magnet_0.__str__().replace(linesep, linesep + "\t").rstrip("\t")
            HoleM64_str += "magnet_0 = " + tmp
        else:
            HoleM64_str += "magnet_0 = None" + linesep + linesep
        return HoleM64_str
    ...
    def __eq__(self, other):
        ...
        if not super(HoleM64, self).__eq__(other):
            return False
        if other.W1 != self.W1:
            return False
        if other.W2 != self.W2:
    ...
    def __sizeof__(self):
        ...
        S += super(HoleM64, self).__sizeof__()
        S += getsizeof(self.W1)
        S += getsizeof(self.W2)
        S += getsizeof(self.W3)
        S += getsizeof(self.H0)
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        # Get the properties inherited from HoleMag
        HoleM64_dict = super(HoleM64, self).as_dict(
            type_handle_ndarray=type_handle_ndarray,
            keep_function=keep_function,
            **kwargs
        )
        HoleM64_dict["W1"] = self.W1
        HoleM64_dict["W2"] = self.W2
        HoleM64_dict["W3"] = self.W3
        HoleM64_dict["H0"] = self.H0
        HoleM64_dict["H2"] = self.H2
        HoleM64_dict["R0"] = self.R0
        if self.magnet_0 is None:
            HoleM64_dict["magnet_0"] = None
        else:
            HoleM64_dict["magnet_0"] = self.magnet_0.as_dict(
                type_handle_ndarray=type_handle_ndarray,
                keep_function=keep_function,
                **kwargs
            )
        ...
        HoleM64_dict["__class__"] = "HoleM64"
        return HoleM64_dict
