...
class MachineInput(FrozenClass):
    """Abstract class to get machine geometrical inputs and materials"""
    ...
    if isinstance(load_machine, ImportError):
        load_machine = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use MachineInput method load_machine: " + str(load_machine)
                )
            )
        )
    else:
    ...
    if isinstance(load_material, ImportError):
        load_material = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use MachineInput method load_material: " + str(load_material)
                )
            )
        )
    else:
    ...
    def __init__(self, init_dict=None, init_str=None):
        ...
        if init_dict is not None:  # Initialisation by dict
            ...
            assert init_dict["__class__"] == "MachineInput"
    ...
    def __str__(self):
        ...
        if self.parent is None:
            MachineInput_str += "parent = None " + linesep
        else:
            MachineInput_str += (
                "parent = " + str(type(self.parent)) + " object" + linesep
            )
        return MachineInput_str
