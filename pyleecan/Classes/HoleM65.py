# -*- coding: utf-8 -*-
# File generated according to Generator/ClassesRef/Slot/HoleM65.csv
# WARNING! All changes made in this file will be lost!
"""Method code available at https://github.com/Eomys/pyleecan/tree/master/pyleecan/Methods/Slot/HoleM65
"""
...
try:
    from ..Methods.Slot.HoleM65._comp_point_coordinate import _comp_point_coordinate
except ImportError as error:
    _comp_point_coordinate = error

try:
    from ..Methods.Slot.HoleM65.build_geometry import build_geometry
except ImportError as error:
    build_geometry = error

try:
    from ..Methods.Slot.HoleM65.plot_schematics import plot_schematics
except ImportError as error:
    plot_schematics = error

try:
    from ..Methods.Slot.HoleM65.remove_magnet import remove_magnet
except ImportError as error:
    remove_magnet = error

try:
    from ..Methods.Slot.HoleM65.comp_surface_magnet_id import comp_surface_magnet_id
except ImportError as error:
    comp_surface_magnet_id = error

try:
    from ..Methods.Slot.HoleM65.comp_radius import comp_radius
except ImportError as error:
    comp_radius = error

try:
    from ..Methods.Slot.HoleM65.has_magnet import has_magnet
except ImportError as error:
    has_magnet = error

try:
    from ..Methods.Slot.HoleM65.check import check
except ImportError as error:
    check = error

try:
    from ..Methods.Slot.HoleM65.comp_magnetization_dict import comp_magnetization_dict
except ImportError as error:
...
class HoleM65(HoleMag):
    ...
    # cf Methods.Slot.HoleM65._comp_point_coordinate
    if isinstance(_comp_point_coordinate, ImportError):
        _comp_point_coordinate = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM65 method _comp_point_coordinate: "
    ...
    # cf Methods.Slot.HoleM65.build_geometry
    if isinstance(build_geometry, ImportError):
        build_geometry = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM65 method build_geometry: " + str(build_geometry)
                )
            )
        )
    else:
        build_geometry = build_geometry
    # cf Methods.Slot.HoleM65.plot_schematics
    if isinstance(plot_schematics, ImportError):
        plot_schematics = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM65 method plot_schematics: " + str(plot_schematics)
                )
            )
        )
    else:
        plot_schematics = plot_schematics
    # cf Methods.Slot.HoleM65.remove_magnet
    if isinstance(remove_magnet, ImportError):
        remove_magnet = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM65 method remove_magnet: " + str(remove_magnet)
                )
            )
        )
    else:
        remove_magnet = remove_magnet
    # cf Methods.Slot.HoleM65.comp_surface_magnet_id
    if isinstance(comp_surface_magnet_id, ImportError):
        comp_surface_magnet_id = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM65 method comp_surface_magnet_id: "
    ...
    # cf Methods.Slot.HoleM65.comp_radius
    if isinstance(comp_radius, ImportError):
        comp_radius = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM65 method comp_radius: " + str(comp_radius))
            )
        )
    else:
        comp_radius = comp_radius
    # cf Methods.Slot.HoleM65.has_magnet
    if isinstance(has_magnet, ImportError):
        has_magnet = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM65 method has_magnet: " + str(has_magnet))
            )
        )
    else:
        has_magnet = has_magnet
    # cf Methods.Slot.HoleM65.check
    if isinstance(check, ImportError):
        check = property(
            fget=lambda x: raise_(
                ImportError("Can't use HoleM65 method check: " + str(check))
            )
        )
    else:
        check = check
    # cf Methods.Slot.HoleM65.comp_magnetization_dict
    if isinstance(comp_magnetization_dict, ImportError):
        comp_magnetization_dict = property(
            fget=lambda x: raise_(
                ImportError(
                    "Can't use HoleM65 method comp_magnetization_dict: "
    ...
    def __str__(self):
        ...
        HoleM65_str += super(HoleM65, self).__str__()
        HoleM65_str += "W1 = " + str(self.W1) + linesep
        HoleM65_str += "W2 = " + str(self.W2) + linesep
        HoleM65_str += "W3 = " + str(self.W3) + linesep
        HoleM65_str += "W4 = " + str(self.W4) + linesep
        HoleM65_str += "H0 = " + str(self.H0) + linesep
        HoleM65_str += "H1 = " + str(self.H1) + linesep
        HoleM65_str += "R = " + str(self.R) + linesep
        if self.magnet_0 is not None:
            tmp = self.magnet_0.__str__().replace(linesep, linesep + "\t").rstrip("\t")
            HoleM65_str += "magnet_0 = " + tmp
        else:
            HoleM65_str += "magnet_0 = None" + linesep + linesep
        if self.magnet_1 is not None:
            tmp = self.magnet_1.__str__().replace(linesep, linesep + "\t").rstrip("\t")
            HoleM65_str += "magnet_1 = " + tmp
        else:
            HoleM65_str += "magnet_1 = None" + linesep + linesep
        return HoleM65_str
    ...
    def __eq__(self, other):
        ...
        if not super(HoleM65, self).__eq__(other):
            return False
        if other.W1 != self.W1:
            return False
        if other.W2 != self.W2:
    ...
    def compare(self, other, name="self", ignore_list=None, is_add_value=False):
        ...
        if (
            other._R is not None
            and self._R is not None
            and isnan(other._R)
            and isnan(self._R)
        ):
            pass
        elif other._R != self._R:
            if is_add_value:
                val_str = " (self=" + str(self._R) + ", other=" + str(other._R) + ")"
                diff_list.append(name + ".R" + val_str)
            else:
                diff_list.append(name + ".R")
    ...
    def __sizeof__(self):
        ...
        S += super(HoleM65, self).__sizeof__()
        S += getsizeof(self.W1)
        S += getsizeof(self.W2)
        S += getsizeof(self.W3)
        S += getsizeof(self.W4)
    ...
    def as_dict(self, type_handle_ndarray=0, keep_function=False, **kwargs):
        ...
        # Get the properties inherited from HoleMag
        HoleM65_dict = super(HoleM65, self).as_dict(
            type_handle_ndarray=type_handle_ndarray,
            keep_function=keep_function,
            **kwargs
        )
        HoleM65_dict["W1"] = self.W1
        HoleM65_dict["W2"] = self.W2
        HoleM65_dict["W3"] = self.W3
        HoleM65_dict["W4"] = self.W4
        HoleM65_dict["H0"] = self.H0
        HoleM65_dict["H1"] = self.H1
        HoleM65_dict["R"] = self.R
        if self.magnet_0 is None:
            HoleM65_dict["magnet_0"] = None
        else:
            HoleM65_dict["magnet_0"] = self.magnet_0.as_dict(
                type_handle_ndarray=type_handle_ndarray,
                keep_function=keep_function,
                **kwargs
            )
        if self.magnet_1 is None:
            HoleM65_dict["magnet_1"] = None
        else:
            HoleM65_dict["magnet_1"] = self.magnet_1.as_dict(
                type_handle_ndarray=type_handle_ndarray,
                keep_function=keep_function,
                **kwargs
            )
        ...
        HoleM65_dict["__class__"] = "HoleM65"
        return HoleM65_dict
    ...
    def _set_R(self, value):
        """setter of R"""
        check_var("R", value, "float", Vmin=0)
        self._R = value
